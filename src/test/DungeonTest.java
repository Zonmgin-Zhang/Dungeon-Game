...
class ModelTest {
	...
	void swordTest() {
		...
		assertEquals(p.getY(), 2);
		// check inventory
		assertFalse(p.hasPotion().isEmpty());
		p.useItem();
		p.moveDown(); 
		assertEquals(p.getX(), 1);
		assertEquals(p.getY(), 2);
	...
	void potionWearOutTest() {
		...
		p.moveUp();
		// before using potion state
		assertTrue(p.hasPotion().equals(new State()));
		p.useItem();
		// after using potion state
		assertTrue(p.hasPotion().equals(new State()));
		try {
	...
	void correctKeyTest() {
		...
		d.addObstacle(r);
		p.moveUp();
		assertFalse(p.hasKey().isEmpty());
		p.moveUp();
		assertTrue(p.hasKey() == true);
